  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!userId) {
          console.error('checkout.session.completed: missing user_id in metadata — cannot link subscription to user')
          break
        }
        ...
        if (session.mode === 'payment') {
          ...
          if (error) console.error('checkout.session.completed (lifetime) upsert error:', error)
          else console.log(`Lifetime subscription activated for user ${userId}`)
          ...
          if (error) console.error('checkout.session.completed (subscription) upsert error:', error)
          else console.log(`Subscription ${plan} activated for user ${userId}`)
        }
        break
      }
