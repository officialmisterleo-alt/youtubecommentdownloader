...
import { BarChart3, Users, ShieldCheck, Building2, Copy } from 'lucide-react'
...
async function fetchEnterpriseData(): Promise<{ teams: EnterpriseTeam[]; pending: PendingProvision[] }> {
  try {
    ...
    // Fetch teams and pending provisions in parallel
  ...
  return (
    <div className="mb-8">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
        <StatCard label="Total Users" value={stats.totalUsers} sub={`+${stats.newUsersThisMonth} this month`} />
        <StatCard
          label="Active Subscriptions"
          value={stats.activeSubscriptions}
          sub={planSummary || undefined}
        />
        <StatCard label="Exports This Month" value={stats.exportsThisMonth} sub={`${stats.totalExports.toLocaleString()} all-time`} />
        <StatCard label="Comments Downloaded" value={stats.totalComments} />
      </div>
              ...
              <tr key={i} className="border-t border-white/[0.05]">
                {Array.from({ length: 7 }).map((__, j) => (
                  <td key={j} className="px-4 py-4">
                    <div className="h-3 bg-white/[0.05] rounded animate-pulse" style={{ width: `${60 + Math.random() * 30}%` }} />
                  </td>
                ))}
              </tr>
            ))}
...
export const metadata = {
  title: 'Admin – YouTubeCommentDownloader',
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="bg-yellow-900/30 text-yellow-400 border border-yellow-900/40 text-xs px-2.5 py-1 rounded-full font-medium">
                admin
              </span>
            </div>
            <p className="text-[#888888] text-sm mt-1">Platform overview · {user.email}</p>
          </div>
          <Link
            href="/dashboard"
            className="text-[#888888] hover:text-[#e5e2e1] text-sm transition-colors"
          >
            ← Dashboard
          </Link>
        </div>
